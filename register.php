...
if ($_SERVER['REQUEST_METHOD'] == "POST"){
    ...
    if(substr($_POST['username'], -12) != "@iiita.ac.in"){
        $extension_err = "Not a valid official email";
        $message = "Not a valid official email";
...
elseif(strlen(trim($_POST['password'])) < 5){
    $message = "Password should be of atleast 5 charecters.";
    echo "<script type='text/javascript'>alert('$message');</script>";
...
if(empty($username_err) && empty($password_err) && empty($confirm_password_err) && empty($extension_err))
{
    ...
    $sql1 = "INSERT INTO technical (username) VALUES (?)";
    ...
    if($stmt)
    {
        ...
        if (mysqli_stmt_execute($stmt))
        {           
            $_SESSION['status'] = "STUDENT REGISTERED SUCESSFULLY";
            //initially it was login.php
